...
Spreadsheet.prototype.handleBatchResult = function(response, body, callback )  {
        //console.log( response.statusCode, response.headers);
        if (response.statusCode != 200 && response.headers["content-type"].match(/^text\/html/ ) ) {
            var matches = body.match(/<title>(.+?)<\/title>/i) || [ null, body ];
            return callback( new Error( response.statusCode, _.unescape( matches[1] ) ) );
        }
        else
        if ( response.headers["content-type"].match(/^text\/xml|^application\/atom\+xml/) ) {
            xmlParseString(body, function(err,result) {
                      if ( ! err && result && result.feed && result.feed.entry && _.isArray(result.feed.entry) ) {
                            //console.error( JSON.stringify(result.feed.entry, null, ' '));
                            var error = result.feed.entry
                                .map( function(v) {
                                    return v["batch:status"] || v["batch:interrupted"] || null  ;
                                    })
                                .filter( function(v) { return _.isArray(v) && v.length ;  }  )
                                .map( function(v) {
                                        return v[0].$
                                 } )
                                .map( function(v) {
                                        if ( ! v.code && v.reason && v.reason.match(/URL could not be found/)) { 
                                            v.code = 404; 
                                        } else {
                                            v.code = _.parseInt( v.code || 400 );
                                        }
                                        return v;
                                 });
                            var rv = error.length ? error[0] : { code: 500 };
                            //console.error( JSON.stringify( error, null, ' '));
                            if ( Math.floor(rv.code/100) !== 2 ) {
                                return callback( new Error(rv.code, rv.reason )) ;
                            } else {
                                return callback(null)
                            }
                      } if ( ! err ) {
                                return callback( new Error( 500, "Unexpected XML response" )) ;
                      } else {
  ...
  async.series([
    ...
    function send(next){
      request({
        ...
        if ( err ) return callback( new Error( 500, err.toString() ) );
        else
        return _this.handleBatchResult( response, body, function(err) {
                if (err ) {
                     _this.log("Response: ", err.toString().red.underline );
                 } else {
