...
function handleBatchResult(response, body, callback ) {
        ...
        if (response.statusCode != 200 && response.headers["content-type"].match(/^text\/html/ ) ) {
            ...
            return callback( new Error( response.statusCode, matches[1] ) );
        }
        else
        if ( response.headers["content-type"].match(/^text\/xml|^application\/atom\+xml/) ) {
            xmlParseString(body, function(err,result) {
                            ...
                            var error = result.feed.entry
                                ...
                                .map( function(v) {
                                        if ( ! v.code && v.reason && v.reason.match(/URL could not be found/)) { v.code = 404; }
                                        return v;
                                 });
                            var rv = error.length ? error[0] : { code: "500" };
                            var code = parseInt(rv.code, 10);
                            if ( Math.floor(code/100) !== 2 ) {
                                return callback( new Error(code, rv.reason )) ;
                            } else {
                                return callback(null)
                            }
                        } );
